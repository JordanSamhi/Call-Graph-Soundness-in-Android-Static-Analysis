...
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.cfgslicer.CFGSlicer;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.config.Config;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.dg.*;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.lifecycle.LifeCycleHandler;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.sdgslicer.FieldCollector;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.sdgslicer.SDGBackwardSlicer;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.sdgslicer.SDGForwardSlicer;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.soot.PDGTransformer;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.soot.ReachingDefinition;
import com.jordansamhi.experiments.callgraphsoundness.from_other_tools.jicer.soot.ValueOrField;
...
public class Jicer {
    ...
    public void init(String platform_path, String apkt_path) {
        // reinitialize
        reset();


        setupSoot(platform_path, apkt_path);
    }
    ...
    private boolean setupSoot(String platform_path, String apkt_path) {
        Options.v().set_src_prec(Options.src_prec_apk);
        Options.v().set_output_format(Options.output_format_none);
        Options.v().set_whole_program(true);
        Options.v().set_android_jars(platform_path);
        Options.v().set_process_multiple_dex(true);
        Options.v().set_exclude(getExcludeList(platform_path, apkt_path));
        Options.v().set_process_dir(
                Collections.singletonList(apkt_path));
    ...
    private List<String> getExcludeList(String platforms, String apkt_path) {
        final ManifestInfo manifestInfo = ManifestHelper.getInstance()
                .getManifest(new File(apkt_path));
        final List<String> excludeList = new ArrayList<>();
        excludeList.addAll(Arrays.asList(new String[]{manifestInfo.getPkgName() + ".BuildConfig",
                manifestInfo.getPkgName() + ".R", manifestInfo.getPkgName() + ".R$*"}));
        if (!Parameters.getInstance().isIncludeOrdinaryLibraryPackages()) {
